...
{
public:
    virtual void paint( MyIconFont* awesome, QPainter* painter, const QRect& rect, QIcon::Mode mode, QIcon::State state, const QVariantMap& options  )
    {
        Q_UNUSED(mode);
        Q_UNUSED(state);
        Q_UNUSED(options);
        ...
        float size = 0.5;
        QRect starRect(rect.left()+1, qRound(rect.top()+rect.height()*(1-size))-1, qRound(rect.width()*size), qRound(rect.height()*size));
        ...
        painter->drawText(starRect, options.value("marker-text").toString(), QTextOption(Qt::AlignCenter|Qt::AlignVCenter));
...
MyIconFont::MyIconFont( QObject* parent )
    : QObject( parent )
    , namedCodepoints_()
{
