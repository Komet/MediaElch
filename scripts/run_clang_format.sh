...
if [[ -x "$(command -v clang-format-14)" ]]; then
	CF=clang-format-14
elif [[ -x "$(command -v clang-format-mp-14)" ]]; then
	# MacPorts version
	CF=clang-format-mp-14
else
	CF=clang-format
	clang-format --version | grep " 14." > /dev/null || (print_warning "WARNING: MediaElch requires clang-format version 14")
fi
