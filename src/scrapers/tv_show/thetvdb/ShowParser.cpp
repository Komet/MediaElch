...
void ShowParser::parseInfos(const QString& json)
{
    ...
    if (m_infosToLoad.contains(ShowScraperInfo::Network)) {
        m_show.setNetwork(helper::mapStudio(showData.value("network").toString()));
    }
    if (m_infosToLoad.contains(ShowScraperInfo::Overview)) {
        m_show.setOverview(showData.value("overview").toString());
    }
    if (m_infosToLoad.contains(ShowScraperInfo::Rating)) {
        Rating rating;
...
void ShowParser::parseImages(const QString& json)
{
    ...
    for (const auto& imageValue : images) {
        ...
        if (keyType == "fanart" && m_infosToLoad.contains(ShowScraperInfo::Fanart)) {
            m_show.addBackdrop(p);

        } else if (keyType == "poster" && m_infosToLoad.contains(ShowScraperInfo::Poster)) {
            m_show.addPoster(p);

        } else if (keyType == "season" && m_infosToLoad.contains(ShowScraperInfo::SeasonPoster)) {
            const auto season = SeasonNumber(imageObj.value("subKey").toString().toInt());
            m_show.addSeasonPoster(season, p);

        } else if (keyType == "seasonwide" && m_infosToLoad.contains(ShowScraperInfo::SeasonBanner)) {
            const auto season = SeasonNumber(imageObj.value("subKey").toString().toInt());
            m_show.addSeasonBanner(season, p);

        } else if (keyType == "series"
                   && (m_infosToLoad.contains(ShowScraperInfo::Banner)
                       || m_infosToLoad.contains(ShowScraperInfo::SeasonBanner))) {
            m_show.addBanner(p);
        }
    }
}
