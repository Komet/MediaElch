    ...
    const QVector<Placeholder>& placeholders)
{
    ...
    for (const auto& placeholder : placeholders) {
        if (!placeholder.isValue && !placeholder.isCondition) {
            qCFatal(generic).nospace() << "There is a placeholder in '" << variant
    ...
    for (const auto& entry : dataKeySet) {
        if (!placeholderSet.contains(entry)) {
            qCFatal(generic).nospace() << "There is a data entry in '" << variant
                                       << "' for which there is no placeholder: " << entry;
    ...
    const QVector<Placeholder>& placeholders)
{
    ...
    for (const auto& entry : conditionKeySet) {
        if (!placeholderSet.contains(entry)) {
            qCFatal(generic).nospace() << "There is a condition entry in '" << variant
                                       << "' for which there is no placeholder: " << entry;
